...
// AllocAt implements the IDAllocator interface.
func (alloc *IDAllocator) AllocAt(id uint64) (bool, error) {
	for {
		base := atomic.LoadUint64(&alloc.base)
		if id <= base {
			return false, nil
		}
		if atomic.CompareAndSwapUint64(&alloc.base, base, id) {
			return true, nil
		}
	}
}
